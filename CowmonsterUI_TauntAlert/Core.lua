...
function f:SPELL_CAST_SUCCESS(self, srcName, srcGUID, dstName, dstGUID, spellID, spellName)
	local msg = nil
	...
	if event == "COMBAT_LOG_EVENT_UNFILTERED" then
		...
		if self[subEvent] then
			if spellID ~= nil and self:IsTauntSpell(spellID) then
				self[subEvent](self, srcName, srcGUID, dstName, dstGUID, spellID, spellName)
			end
		end
	end
end
