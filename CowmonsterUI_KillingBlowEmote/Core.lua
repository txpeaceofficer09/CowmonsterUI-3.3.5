...
local function OnEvent(self, event, ...)
	...
	if subEvent == "PARTY_KILL" then
		if srcGUID == UnitGUID("player") and ( bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) > 0 and bit.band(dstFlags, COMBATLOG_OBJECT_TYPE_PLAYER) > 0 ) and not ( bit.band(dstFlags, COMBATLOG_OBJECT_AFFILIATION_RAID) > 0 or bit.band(dstFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY) > 0 or bit.band(dstFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) > 0 ) then
			if hasAchievement == true then -- Check if you already have the achievement for hugging a fallen enemy.
	...
	elseif subEvent == "UNIT_DIED" then
		if ( dstFlags ~= nil and bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) > 0 and bit.band(dstFlags, COMBATLOG_OBJECT_TYPE_PLAYER) > 0 ) then
			if hasAchievement == false then -- You do not have the achievement and someone killed an enemy player.
				DoEmote("HUG", dstName) -- Attempt to get the achievement by hugging the corpse.
			end
		end
	end
end
